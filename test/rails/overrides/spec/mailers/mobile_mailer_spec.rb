...
describe MobileMailer, 'receiving', type: :mailer do
  describe 'blank mail' do
    it 'softbank からの空メールがで受信できること' do
      email = file_fixture('mobile_mailer/softbank-blank.eml').read
      expect {
  ...
  describe 'docomo からのメールを受信するとき' do
    before(:each) do
      @email = file_fixture('mobile_mailer/docomo-emoji.eml').read
    end
    ...
    describe 'jis コードの場合に' do
      before(:each) do
        @email = file_fixture('mobile_mailer/docomo-jis.eml').read
      end
  ...
  describe 'au からのメールを受信するとき' do
    describe 'jpmobile で送信したメールの場合' do
      before(:each) do
        @email = file_fixture('mobile_mailer/au-emoji.eml').read
      end
    ...
    describe '実機からのメールの場合' do
      before(:each) do
        @email = file_fixture('mobile_mailer/au-emoji2.eml').read
      end
  ...
  describe 'softbank からのメールを受信するとき' do
    describe 'shift_jis のとき' do
      before(:each) do
        @email = file_fixture('mobile_mailer/softbank-emoji.eml').read
      end
    ...
    describe 'utf-8 のとき' do
      before(:each) do
        @email = file_fixture('mobile_mailer/softbank-emoji-utf8.eml').read
      end
  ...
  describe 'multipart メールを受信するとき' do
    describe 'docomo の場合' do
      ...
      before(:each) do
        @email = file_fixture('mobile_mailer/docomo-gmail-sjis.eml').read
      end
    ...
    describe 'au の場合' do
      before(:each) do
        @email = file_fixture('mobile_mailer/au-decomail.eml').read
      end
    ...
    describe 'softbank(sjis) の場合' do
      # NOTE: キャリアメールサーバで絵文字を変換するため検証は困難
      before(:each) do
        @email = file_fixture('mobile_mailer/softbank-gmail-sjis.eml').read
      end
    ...
    describe 'softbank(utf8) の場合' do
      # NOTE: キャリアメールサーバで絵文字を変換するため検証は困難
      before(:each) do
        @email = file_fixture('mobile_mailer/softbank-gmail-utf8.eml').read
      end
    ...
    describe '添付ファイルがある場合' do
      # NOTE: au のみテスト
      before(:each) do
        @email = file_fixture('mobile_mailer/au-attached.eml').read
      end
