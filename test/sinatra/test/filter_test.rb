...
class SinatraOnJpmobile < Test::Unit::TestCase
  ...
  def test_docomo_get_convert_to_utf8
    get '/', {:g => to_sjis("万葉")}, {"HTTP_USER_AGENT" => "DoCoMo/2.0 SH902i(c100;TB;W24H12)"}
    assert_equal last_app.assigns(:g), "万葉"
    assert_equal last_response.body, to_sjis("万葉")
  end

  def test_docomo_post_convert_to_utf8
    post '/', {:p => to_sjis("けーたい")}, {"HTTP_USER_AGENT" => "DoCoMo/2.0 SH902i(c100;TB;W24H12)"}
    assert_equal last_app.assigns(:p), "けーたい"
    assert_equal last_response.body, to_sjis("けーたい")
  end
end
